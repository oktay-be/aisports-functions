...
Processes files SIMULTANEOUSLY using asyncio tasks for concurrent execution.
...
GCS Files Processed Concurrently:
...
class AISummarizerGCSMultiThreaded:
    """
    Tests AI Summarizer using session data stored in Google Cloud Storage with concurrent processing.
    Uses the same structure and patterns as the main AISummarizer class but with asyncio task management.
    """
    ...
    def __init__(self, project_id: str = None, location: str = None, model_name: str = "gemini-2.5-pro"):
        ...
        try:
            ...
            logger.info(f"AISummarizerGCSMultiThreaded initialized with Vertex AI: project={self.project_id}, location={self.location}, model={model_name}")
        except Exception as e:
            logger.error(f"Failed to initialize Vertex AI client: {e}", exc_info=True)
            self.client = None

    async def process_gcs_session_data_task(self, gcs_uri: str, task_id: int) -> Tuple[str, Dict[str, Any], int]:
        """
        Process session data directly from Google Cloud Storage using Vertex AI as an asyncio task.
        Returns a tuple with (gcs_uri, result, task_id) for easy identification in concurrent processing.
        ...
        Returns:
            Tuple[str, Dict[str, Any], int]: (gcs_uri, processed_result, task_id)
        """
        if not self.client:
            logger.error(f"Task {task_id}: Vertex AI client is not initialized. Cannot process GCS data.")
            return gcs_uri, {
        ...
        try:
            ...
            if not prompt_md_path.exists():
                logger.error(f"Task {task_id} ({source_domain}): PROMPT.md file not found: {prompt_md_path}")
                return gcs_uri, {
...
The data contains European sports news articles that need to be processed according to the OUTPUT FORMAT specified in the prompt above. Return the structured JSON result."""
            ...
            logger.info(f"Task {task_id}: Processing session data from GCS: {source_domain}")
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.info(f"Task {task_id} ({source_domain}): AI operation completed in {ai_duration:.2f}s (attempt {attempt + 1})")
                    ...
                    try:
                        if os.getenv("STRUCTURED_OUTPUT", "true").lower() == "true":
                            ...
                            if hasattr(response, 'parsed') and response.parsed:
                                logger.info(f"Task {task_id} ({source_domain}): Parsing structured response")
                                result = response.parsed
                            else:
                                # Fallback: try to parse from response.text with langchain-core
                                logger.warning(f"Task {task_id} ({source_domain}): No parsed response available, falling back to text parsing")
                                clean_json = parse_json_markdown(response.text)
                                result = clean_json
                        else:
                            # For unstructured output, always use langchain-core to clean the response
                            logger.info(f"Task {task_id} ({source_domain}): Parsing unstructured response using langchain-core")
                            clean_json = parse_json_markdown(response.text)
                            result = clean_json
                            
                    except Exception as parse_error:
                        logger.error(f"Task {task_id} ({source_domain}): Failed to parse response content: {parse_error}")
                        return gcs_uri, {
                    ...
                    # Record response usage_metadata - same pattern with task-specific filename
                    await self._save_usage_metadata(response, task_id, source_domain)
                    ...
                    if "429" in str(e) or "quota" in str(e).lower() or "resource_exhausted" in str(e).lower():
                        if attempt < max_retries:
                            ...
                            logger.warning(f"Task {task_id} ({source_domain}): Quota exceeded. Retrying in {delay}s (attempt {attempt + 1}/{max_retries + 1})")
                            await asyncio.sleep(delay)
                            continue
                        else:
                            logger.error(f"Task {task_id} ({source_domain}): Max retries exceeded. Check Vertex AI quotas and billing.")
                            raise
                    else:
            ...
            if "processing_summary" in result and "processed_articles" in result:
                ...
                logger.info(f"Task {task_id} ({source_domain}): Successfully processed GCS session data: {processed_count} articles processed")
                
                # Save the GCS result to workspace with task-specific information
                await self._save_gcs_result_to_workspace(gcs_uri, result, task_id, source_domain)
                
                return gcs_uri, result, task_id
            else:
                logger.error(f"Task {task_id} ({source_domain}): Response structure validation failed. Keys: {list(result.keys())}")
                return gcs_uri, {
            ...
            logger.error(f"Task {task_id} ({source_domain}): File not found error: {e}")
            return gcs_uri, {
            ...
            logger.error(f"Task {task_id} ({source_domain}): Error during GCS session data processing: {e}", exc_info=True)
            return gcs_uri, {
    ...
    async def _save_usage_metadata(self, response, task_id: int, source_domain: str) -> None:
        """
        Save usage metadata with task-specific information.
        """
        try:
            ...
            if hasattr(response, 'usage_metadata') and response.usage_metadata:
                ...
                debug_file = workspace_debug / f"gcs_mt_usage_metadata_task{task_id}_{source_domain}_{timestamp}.json"
                
                with open(debug_file, 'w', encoding='utf-8') as f:
                    json.dump(response.usage_metadata, f, indent=2, ensure_ascii=False, default=str)
                
                logger.info(f"Task {task_id} ({source_domain}): Usage metadata saved to: {debug_file}")
            else:
                logger.warning(f"Task {task_id} ({source_domain}): No usage_metadata found in response")
                
        except Exception as debug_error:
            logger.warning(f"Task {task_id} ({source_domain}): Failed to save usage metadata: {debug_error}")

    async def _save_gcs_result_to_workspace(self, gcs_uri: str, result: Dict[str, Any], task_id: int, source_domain: str) -> None:
        """
        Save GCS processing result to workspace with task-specific information.
        """
        try:
            workspace_debug = Path(__file__).parent / ".workspace" / "debug"
            workspace_debug.mkdir(parents=True, exist_ok=True)
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            result_filename = f"gcs_mt_result_task{task_id}_{gcs_filename}_{timestamp}.json"
            ...
            logger.info(f"Task {task_id} ({source_domain}): GCS processing result saved to: {result_path}")
            
        except Exception as e:
            logger.error(f"Task {task_id} ({source_domain}): Failed to save GCS result to workspace: {e}", exc_info=True)


async def test_gcs_session_data_multi_threaded():
    """
    Test processing session data from Google Cloud Storage using concurrent asyncio tasks.
    Processes multiple GCS files simultaneously for faster execution.
    """
    logger.info("🚀 Starting Multi-Threaded GCS Session Data Test")
    
    # Initialize GCS-enabled AI Summarizer
    summarizer = AISummarizerGCSMultiThreaded()
    
    if not summarizer.client:
        logger.error("❌ Failed to initialize AISummarizerGCSMultiThreaded")
        return False
    
    # GCS file locations to process concurrently
    gcs_files = [
        "gs://multi-modal-ai-bucket/session_data_fanatik_com_tr.json",
        "gs://multi-modal-ai-bucket/session_data_fotomac_com_tr.json", 
        "gs://multi-modal-ai-bucket/session_data_milliyet_com_tr.json",
    ...
    # Process files concurrently using asyncio tasks
    logger.info(f"🚀 Starting concurrent processing of {len(gcs_files)} GCS files...")
    logger.info("Note: Using concurrent processing for faster execution")
    start_time = asyncio.get_event_loop().time()
    
    # Create tasks for all GCS files
    tasks = []
    for i, gcs_uri in enumerate(gcs_files):
        filename = Path(gcs_uri).stem
        source_domain = filename.replace('session_data_', '').replace('_', '.')
        logger.info(f"📝 Creating task {i+1}/{len(gcs_files)}: {source_domain}")
        
        # Create and append task
        task = asyncio.create_task(
            summarizer.process_gcs_session_data_task(gcs_uri, task_id=i+1),
            name=f"gcs_processing_task_{i+1}_{source_domain}"
        )
        tasks.append(task)
    
    logger.info(f"✅ Created {len(tasks)} concurrent tasks. Starting execution...")
    
    # Gather all tasks and wait for completion
    try:
        results = await asyncio.gather(*tasks, return_exceptions=True)
    except Exception as e:
        logger.error(f"❌ Error during concurrent task execution: {e}")
        return False
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            gcs_uri, processing_result, task_id = result
            filename = Path(gcs_uri).stem
            source_domain = filename.replace('session_data_', '').replace('_', '.')
            
            if processing_result.get("error"):
                logger.error(f"❌ Task {task_id} ({source_domain}) failed: {processing_result['error']}")
                failed_results.append((task_id, processing_result['error']))
            else:
                processed_articles = len(processing_result.get('processed_articles', []))
                logger.info(f"✅ Task {task_id} ({source_domain}) completed successfully: {processed_articles} articles processed")
    ...
    # Log final results
    logger.info(f"Concurrent processing completed after {total_duration:.1f}s total")
    ...
    for processed in processed_results:
        ...
        logger.info(f"  ✅ Task {task_id}: {source_domain} -> {articles_processed} articles processed")
    
    for task_id, error in failed_results:
...
async def main():
    """Main test function - multi-threaded version."""
    logger.info("=" * 80)
    logger.info("🧪 Multi-Threaded GCS Session Data Processing Test")
    logger.info("=" * 80)
    ...
    # Run the test
    success = await test_gcs_session_data_multi_threaded()
    
    if success:
        logger.info("✅ Multi-threaded GCS session data test completed successfully!")
    else:
        logger.error("❌ Multi-threaded GCS session data test failed!")
